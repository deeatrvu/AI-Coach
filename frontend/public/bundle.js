...
(() => {
  ...
  var PersonaManager = class {
    ...
    async loadPersonas() {
      ...
      const sel = (0, import_jquery.default)("#persona-select");
      sel.empty();
      this.personas.forEach((p) => {
        const name = p.description.split(" \u2014 ")[0];
        sel.append((0, import_jquery.default)("<option>").val(p.id).text(name));
      });
      sel.on("change", () => {
  ...
  var FeedbackRenderer = class {
    showFeedback(evaluation) {
      (0, import_jquery.default)("#feedback").prop("hidden", false);
      (0, import_jquery.default)("#feedback-json").text(JSON.stringify(evaluation, null, 2));
    }
    renderScores(scores) {
    }
  };
  ...
  var RealtimeClient = class {
    constructor(apiBase = window.API_BASE || "http://localhost:8000/api") {
      ...
      this.mood = "Neutral";
      this.timePressureLevel = 4;
      // start very high: busy senior doctor
      this.skepticismLevel = 4;
      // start very skeptical
      this.lastToneUpdateAt = 0;
      this.minToneUpdateMs = 900;
      this.patienceScore = 1;
      // start with very low patience (0-5)
      this.engagementScore = 1;
      // start with very low engagement (0-5)
      this.hypeCount = 0;
      // track hype phrases
      this.evidenceCount = 0;
      // track evidence provided
      this.monologueCount = 0;
      // track long monologues
      this.cutNowTriggered = false;
      // track if doctor wants to end call
      this.hangupTimer = null;
      // timer for auto-hangup
      ...
      this.vadHighThreshold = 0.08;
      // RMS
      this.vadLowThreshold = 0.03;
      // RMS
      this.vadHoldMs = 250;
      // hysteresis
      this.vadLastAboveTs = 0;
      this.remoteVolumeBeforeDuck = 1;
    }
    setCoachingCallback(callback) {
      this.coachingCallback = callback;
    }
    async start() {
      this.audioEl = document.getElementById("doctor-audio");
      ...
      this.dc.onmessage = (event) => {
        try {
          ...
          if (data.type === "conversation.item.created" && (data.item?.type === "input" || data.item?.type === "message") && data.item?.role === "user") {
            const content = data.item.content?.[0]?.text || "";
    ...
    async mirrorTranscriptToBackend() {
      if (!this.sessionId) return;
      try {
        const url = `${this.apiBase}/transcript/${this.sessionId}/add`;
        const latestMessages = this.transcript.slice(-2);
    ...
    async evaluateSession(transcript, personaId = "doc_001") {
    ...
    updateToneAndNudge(signals, content) {
    ...
    async requestBackendToneUpdate(lastDoctorMessage) {
      const now = Date.now();
      if (now - this.lastToneUpdateAt < this.minToneUpdateMs) return;
    ...
    setupMicProcessing() {
      try {
        ...
        const loop = () => {
          ...
          (0, import_jquery2.default)("#mic-level").text(`${pct}%`);
    ...
    mute(toggle) {
      if (typeof toggle === "boolean") this.isMuted = toggle;
  ...
  var CoachingOverlay = class {
    constructor() {
      this.overlay = (0, import_jquery3.default)("#coaching-overlay");
      this.hint = (0, import_jquery3.default)("#coaching-hint");
      this.hintText = this.hint.find(".hint-text");
      this.hintIcon = this.hint.find(".hint-icon");
    }
    showHint(message, type = "info", icon = "\u{1F4A1}") {
    ...
    showEvidenceHint() {
      this.showHint("\u{1F4A1} Provide specific trial data, patient outcomes, or guideline references", "warning", "\u{1F4CA}");
    }
    showDeEscalateHint() {
    ...
    showTimePressureHint() {
      this.showHint("\u23F0 Doctor seems pressed for time - be concise and focus on key benefits", "warning", "\u23F0");
    }
    showObjectionHint() {
      this.showHint("\u{1F914} Doctor raised an objection - address it directly with evidence", "info", "\u{1F914}");
    }
    showClosureHint() {
      this.showHint("\u{1F3AF} Doctor seems ready to end - summarize key points and next steps", "info", "\u{1F3AF}");
    }
    showStartHint() {
      this.showHint("\u{1F3A4} Start your conversation - introduce yourself and the medication", "info", "\u{1F3A4}");
    }
    showGenericHint(message) {
      this.showHint(message, "info", "\u{1F4A1}");
    }
    // Analyze doctor response and show appropriate hint
    analyzeDoctorResponse(response, signals = []) {
    ...
    showStageHint(stage) {
      switch (stage) {
        case "Introduction":
          this.showHint("\u{1F44B} Introduce yourself and establish rapport", "info", "\u{1F44B}");
          break;
        case "Discussion":
          this.showHint("\u{1F4AC} Present your medication benefits with evidence", "info", "\u{1F4AC}");
          break;
        case "ObjectionDiscussion":
          this.showHint("\u{1F6E1}\uFE0F Address objections with data and patient stories", "warning", "\u{1F6E1}\uFE0F");
          break;
        case "Closure":
          this.showHint("\u{1F3AF} Summarize key points and propose next steps", "info", "\u{1F3AF}");
          break;
        default:
  ...
  var feedback = new FeedbackRenderer();
  var rtc = new RealtimeClient();
  var coaching = new CoachingOverlay();
  async function init() {
    (0, import_jquery4.default)("#session-status").text("Connecting...");
    await personaManager.loadPersonas();
    (0, import_jquery4.default)("#session-status").text("Ready");
    rtc.setCoachingCallback((content, type, signals) => {
      coaching.analyzeDoctorResponse(content, signals);
    });
    (0, import_jquery4.default)("#rt-start").on("click", async () => {
      try {
        (0, import_jquery4.default)("#session-status").text("Starting voice...");
        coaching.showStartHint();
        await rtc.start();
        (0, import_jquery4.default)("#rt-start").prop("disabled", true);
        (0, import_jquery4.default)("#rt-stop").prop("disabled", false);
        (0, import_jquery4.default)("#session-status").text("Voice connected");
      } catch (e) {
        (0, import_jquery4.default)("#session-status").text("Error");
        alert(e.message || e.toString());
      }
    });
    (0, import_jquery4.default)("#rt-stop").on("click", async () => {
      try {
        await rtc.stop();
        coaching.hide();
        ...
        (0, import_jquery4.default)("#rt-start").prop("disabled", false);
        (0, import_jquery4.default)("#rt-stop").prop("disabled", true);
        (0, import_jquery4.default)("#session-status").text("Voice stopped - Evaluation complete");
      } catch (e) {
        alert(e.message || e.toString());
      }
    });
